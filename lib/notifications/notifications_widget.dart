...
class _NotificationsWidgetState extends State<NotificationsWidget> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.apiResulto0f = await TechnicienGroup.tasksCall.call(appState.authToken);

      if ((_model.apiResulto0f?.succeeded ?? true)) {
        _model.tasksNotifications = ((_model.apiResulto0f?.jsonBody ?? '').toList().map<TechnicianTaskStruct?>(TechnicianTaskStruct.maybeFromMap).toList() as Iterable<TechnicianTaskStruct?>)
            .withoutNulls
            .toList()
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        body: Builder(
          builder: (context) {
            final notificationsTasks = _model.tasksNotifications.toList().take(20).toList();
            if (_isloadingData) {
              return Center(
                child: Column(
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, notificationsTasksIndex) {
                final notificationsTasksItem = notificationsTasks[notificationsTasksIndex];
                return wrapWithModel(
                  ...
                  child: NotifsWidget(
                    ...
                    notificationStatut: notificationsTasksItem.notifificationStatus,
                    uuID: notificationsTasksItem.uti_utilsateur_id,
                    city: valueOrDefault<String>(
                      notificationsTasksItem.cityName,
                      'Ville',
                    ),
                    date: notificationsTasksItem.date_previsionnelle_debut,
                    id: notificationsTasksItem.id,
                    datePD: notificationsTasksItem.date_previsionnelle_debut,
                    dateAF: notificationsTasksItem.date_affectation,
                    ...
                    simPropietaire: notificationsTasksItem.SIM_proprietaire,
                    ...
                    gpsPrincipal: notificationsTasksItem.gps_principale,
                    ...
                    telClient: notificationsTasksItem.clientPhoneNumber,
                    nameModelSIM: notificationsTasksItem.nameModeleSIM,
                    colorIndicator: notificationsTasksItem.notifificationStatus == 0 ? Color(0xA48B9898) : FlutterFlowTheme.of(context).secondary,
                    contentViewBG: notificationsTasksItem.notifificationStatus == 0 ? Color(0x96F1F4F8) : FlutterFlowTheme.of(context).secondaryBackground,
